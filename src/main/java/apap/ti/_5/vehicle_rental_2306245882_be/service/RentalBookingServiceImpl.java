...
public class RentalBookingServiceImpl implements RentalBookingService {
  ...
  public RentalBooking createRentalBooking(CreateRentalBookingRequestDTO req) {
    ...
    java.util.concurrent.atomic.AtomicReference<String> chosenVehicleRef = new java.util.concurrent.atomic.AtomicReference<>(
      null
    );
    ...
    Vehicle vehicle = vehicleRepository
      .findById(chosenVehicleId)
      .orElseThrow(() ->
        new BadRequestException("Vehicle not found: " + chosenVehicleId)
      );
    ...
    if (
      req.getCapacityNeeded() != null &&
      vehicle.getCapacity() != null &&
    ...
    if (vehicle.getRentalVendor() != null) {
      ...
      List<String> locations = vendor.getListOfLocations() == null
        ? Collections.emptyList()
        : vendor.getListOfLocations();
      if (
        !locations.contains(req.getPickUpLocation()) ||
        !locations.contains(req.getDropOffLocation())
      ) {
        throw new BadRequestException(
          "Vendor doesn't operate in given pickup/dropoff locations"
        );
      }
    }

    // cek overlapping booking (pakai chosenVehicleId)
    List<RentalBooking> overlapping = bookingRepository.findOverlappingBookings(
      chosenVehicleId,
      req.getPickUpTime(),
      req.getDropOffTime()
    );
    ...
    long hours = Duration
      .between(req.getPickUpTime(), req.getDropOffTime())
      .toHours();
    if (hours <= 0) throw new BadRequestException(
    ...
    double driverCost = (
        req.getIncludeDriver() != null && req.getIncludeDriver()
      )
      ? (days * DRIVER_COST_PER_DAY)
      : 0.0;
    ...
    if (req.getAddonIds() != null && !req.getAddonIds().isEmpty()) {
      ...
      try {
        ...
        throw new BadRequestException(
          "One or more addonIds are not valid UUIDs"
        );
      }
    ...
    RentalBooking booking = RentalBooking
      ...
      .includeDriver(
        req.getIncludeDriver() == null ? false : req.getIncludeDriver()
      )
  ...
  @Override
  public List<SearchResultVehicleDTO> findAvailableVehicles(
    CreateRentalBookingRequestDTO req
  ) {
    ...
    if (days <= 0) throw new BadRequestException(
    ...
    for (Vehicle v : all) {
      if (
        v.getStatus() == null || !v.getStatus().equalsIgnoreCase("Available")
      ) continue;
      if (
        req.getCapacityNeeded() != null &&
        (v.getCapacity() == null || v.getCapacity() < req.getCapacityNeeded())
      ) continue;
      if (
        req.getTransmissionNeeded() != null &&
        v.getTransmission() != null &&
        !v.getTransmission().equalsIgnoreCase(req.getTransmissionNeeded())
      ) continue;
      // vendor locations
      if (v.getRentalVendor() != null) {
        ...
        if (
          vendorLocs == null ||
          !vendorLocs.contains(req.getPickUpLocation()) ||
          !vendorLocs.contains(req.getDropOffLocation())
        ) {
          continue;
        }
      }
  ...
  public RentalBooking getById(String id) {
    return bookingRepository
      .findById(id)
      .orElseThrow(() ->
        new ResourceNotFoundException("RentalBooking not found with id: " + id)
      );
  }

  @Transactional
  @Override
  public RentalBooking updateBookingDetails(UpdateRentalBookingRequestDTO dto)
    ...
    if (!"Upcoming".equalsIgnoreCase(status)) {
      throw new BadRequestException(
        "Booking hanya dapat diubah ketika status 'Upcoming'."
      );
    }

    // 3) validasi waktu
    if (
      dto.getPickUpTime() == null ||
      dto.getDropOffTime() == null ||
      !dto.getPickUpTime().isBefore(dto.getDropOffTime())
    ) {
      throw new BadRequestException(
        "Pick-up time harus sebelum Drop-off time."
      );
    }
    ...
    if (ov.isEmpty()) throw new BadRequestException(
      "Vehicle tidak ditemukan: " + dto.getVehicleId()
    );
    Vehicle vehicle = ov.get();

    // 5) cek overlapping bookings (pakai repo yang ada, lalu exclude current booking)
    List<RentalBooking> overlaps = bookingRepository.findOverlappingBookings(
      dto.getVehicleId(),
      dto.getPickUpTime(),
      dto.getDropOffTime()
    );
    if (overlaps != null && !overlaps.isEmpty()) {
      ...
      if (!overlaps.isEmpty()) {
        throw new BadRequestException(
          "Kendaraan sudah dipesan pada rentang waktu yang dipilih."
        );
      }
    }
    ...
    double driverCost = (
        dto.getIncludeDriver() != null && dto.getIncludeDriver()
      )
      ? (days * DRIVER_COST_PER_DAY)
      : 0.0;
    ...
    booking.setIncludeDriver(
      dto.getIncludeDriver() == null ? false : dto.getIncludeDriver()
    );
    // set transmissionNeeded (entity uses transmissionNeeded in create)
    try {
      booking.setTransmissionNeeded(dto.getTransmission());
    } catch (Exception ignored) {}
  ...
  public RentalBooking updateBookingStatus(String id, String newStatus) {
    RentalBooking booking = bookingRepository
      .findById(id)
      .orElseThrow(() ->
        new BadRequestException("Booking tidak ditemukan: " + id)
      );

    BookingStatus current = booking.getStatus();
    if (current == null) {
      throw new BadRequestException(
        "Booking memiliki status yang tidak valid."
      );
    }
    ...
    try {
      ...
      throw new BadRequestException(
        "Status target tidak dikenal: " + newStatus
      );
    }
    ...
    if (!allowed) {
      throw new BadRequestException(
        "Transisi status tidak diizinkan: " + current + " -> " + target
      );
    }
    ...
    if (target == BookingStatus.Ongoing && booking.getPickUpTime() != null) {
      if (booking.getPickUpTime().isAfter(java.time.LocalDateTime.now())) {
        throw new BadRequestException(
          "Tidak bisa set ONGOING sebelum waktu pick-up."
        );
      }
    }
  ...
  @Override
  @Transactional
  public RentalBooking updateAddOns(UpdateRentalBookingRequestDTO dto)
    throws BadRequestException {
  ...
  public RentalBooking cancelBooking(String id) throws BadRequestException {
    ...
    if (
      ...
      booking.getStatus() != RentalBooking.BookingStatus.Upcoming
    ) {
      throw new BadRequestException(
        "Booking hanya dapat dibatalkan ketika status 'Upcoming'."
      );
    }
