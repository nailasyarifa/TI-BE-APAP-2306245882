...
public class RentalBookingController {
  ...
  public RentalBookingController(
    RentalBookingService bookingService,
  ...
  public String showCreateForm(
    ...
    Set<String> locations = vendors
      .stream()
      .filter(Objects::nonNull)
      .flatMap(v ->
        v.getListOfLocations() == null
          ? Stream.<String>empty()
          : v.getListOfLocations().stream()
      )
  ...
  @PostMapping("/create")
  public String handleCreateOrSearch(
    @ModelAttribute("createReq") CreateRentalBookingRequestDTO req,
    Model model,
    ...
    if (req.getVehicleId() == null || req.getVehicleId().isBlank()) {
      try {
        ...
        ra.addFlashAttribute(
          "error",
          "Kesalahan saat mencari kendaraan: " + e.getMessage()
        );
        ra.addFlashAttribute("createReq", req);
        return "redirect:/bookings/create";
      }
    ...
    try {
      ...
      ra.addFlashAttribute(
        "success",
        "Booking berhasil dibuat: " + saved.getId()
      );
      ...
      ra.addFlashAttribute(
        "error",
        "Kesalahan saat menyimpan booking: " + e.getMessage()
      );
      ra.addFlashAttribute("createReq", req);
      return "redirect:/bookings/create";
    }
  ...
  public String proceedToAddOns(
    ...
    if (vid == null || vid.isBlank()) {
      ra.addFlashAttribute(
        "error",
        "Pilih kendaraan terlebih dahulu sebelum melanjutkan."
      );
      ra.addFlashAttribute("createReq", req);
      return "redirect:/bookings/create";
    }
  ...
  public String showAddOns(
    ...
    if (
      flashedReq.getVehicleId() != null && !flashedReq.getVehicleId().isBlank()
    ) {
      try {
        Optional<Vehicle> vopt = vehicleRepository.findById(
          flashedReq.getVehicleId()
        );
  ...
  public String showUpdateDetailsForm(
    ...
    try {
      ...
      ra.addFlashAttribute(
        "error",
        "Tidak dapat membuka form update: " + e.getMessage()
      );
      return "redirect:/bookings/" + id;
    }
  }
  ...
  public String handleUpdateDetails(
    @ModelAttribute("updateReq") UpdateRentalBookingRequestDTO dto,
    RedirectAttributes ra
  ) {
    try {
      ...
      ra.addFlashAttribute(
        "success",
        "Booking berhasil diperbarui: " + saved.getId()
      );
      ...
      ra.addFlashAttribute(
        "error",
        "Kesalahan saat menyimpan perubahan: " + e.getMessage()
      );
      ra.addFlashAttribute("updateReq", dto);
      return "redirect:/bookings/" + dto.getId() + "/update-details";
    }
  ...
  public String handleUpdateStatus(
    ...
    try {
      ...
      ra.addFlashAttribute(
        ...
        (updated.getStatus() != null ? updated.getStatus().name() : status)
      );
    } catch (BadRequestException e) {
      ra.addFlashAttribute("error", e.getMessage());
      return "redirect:/bookings/" + id + "/status";
  ...
  public String handleUpdateStatus(
    @ModelAttribute("statusReq") UpdateStatusRequestDTO dto,
    RedirectAttributes ra
  ) {
    try {
      ...
      ra.addFlashAttribute(
        "success",
        "Status booking diperbarui: " + updated.getStatus()
      );
      ...
      ra.addFlashAttribute(
        "error",
        "Kesalahan saat memperbarui status: " + e.getMessage()
      );
      ra.addFlashAttribute("statusReq", dto);
      return "redirect:/bookings/" + dto.getId() + "/status";
    }
  ...
  public String showUpdateAddOnsForm(
    @PathVariable("id") String id,
    Model model,
    RedirectAttributes ra
  ) {
    ...
    if (booking.getStatus() != RentalBooking.BookingStatus.Upcoming) {
      ra.addFlashAttribute(
        "error",
        "Hanya booking dengan status UPCOMING yang bisa mengubah Add-Ons."
      );
      return "redirect:/bookings/" + id;
    }
  ...
  public String handleUpdateAddOns(
    ...
    try {
      ...
      ra.addFlashAttribute(
        "error",
        "Format tanggal/waktu tidak valid: " + ex.getMessage()
      );
    ...
    try {
      ...
      ra.addFlashAttribute(
        "error",
        "Terjadi kesalahan saat menyimpan: " + ex.getMessage()
      );
      ra.addFlashAttribute("updateReq", dto);
      return "redirect:/bookings/" + id + "/update-addons";
    }
