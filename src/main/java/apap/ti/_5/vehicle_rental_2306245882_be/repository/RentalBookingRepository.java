...
public interface RentalBookingRepository
  extends JpaRepository<RentalBooking, String> {
  /**
   ...
   */
  @Query(
    "SELECT CASE WHEN COUNT(b) > 0 THEN true ELSE false END FROM RentalBooking b " +
  ...
  @Query(
    "SELECT b FROM RentalBooking b WHERE b.vehicleId = :vehicleId " +
  ...
  @Query(
    "SELECT rb FROM RentalBooking rb WHERE rb.deleted = false OR rb.deleted IS NULL"
  )
